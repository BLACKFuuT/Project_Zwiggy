...
class OrderRepository:
    ...
    async def create_order(self, db: AsyncSession, order: Order) -> Order:
        db.add(order)
        await db.flush()  # assigns id to order
        return order

    async def create_order_item(self, db: AsyncSession, order_item: OrderItem) -> OrderItem:
        db.add(order_item)
        await db.flush()
        return order_item
    ...
    async def update_order_total(self, db: AsyncSession, order_id: int, total: float):
        await db.execute(
            update(Order)
    ...
    async def update_status(self, db: AsyncSession, order_id: int, status: str):
        await db.execute(
            update(Order)
            .where(Order.id == order_id)
